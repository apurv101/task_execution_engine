...
class LLMInterface:
    ...
    async def generate_next_action(
        ...
        # Append the prompt to a file for debugging or logging
        ...
        # If an image is provided, convert it to base64 and append to the payload
        if image:
            buffered = io.BytesIO()
        ...
        async with httpx.AsyncClient() as client:
            async_response = await client.post(url=self.api_url, headers=self.headers, json=payload, timeout=100)
        
        if async_response.status_code != 200:
            print(f"Error: {async_response.status_code} - {async_response.text}")
            return None
        
        completion = async_response.json()
